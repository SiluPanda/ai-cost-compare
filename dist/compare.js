...
exports.compareSavings = compareSavings;
...
function calculateModelCost(model, inputTokens, outputTokens) {
    const pricing = (0, pricing_1.getModelPricing)(model);
    if (!pricing) {
        return undefined;
    }
...
function resolveModels(options) {
    ...
    return (0, pricing_1.listModels)();
}
/**
 * Apply constraints to filter model results.
 */
function applyConstraints(breakdowns, options) {
 ...
 */
function rankModels(breakdowns) {
 ...
 */
function calculateSavings(from, to) {
 ...
 */
function compare(prompt, options) {
    const models = resolveModels(options);
    const inputTokens = (0, tokenizer_1.estimatePromptTokens)(prompt);
 ...
 */
function compareSavings(modelA, modelB, prompt, estimatedOutputTokens = 0) {
    const inputTokens = (0, tokenizer_1.estimatePromptTokens)(prompt);
